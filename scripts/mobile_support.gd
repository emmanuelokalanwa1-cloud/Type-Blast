...
func _ready() -> void:
	...
	# Allow free rotation instead of locking to portrait — the device
	# reports whichever way it's actually being held, and _on_resized
	# below picks that up and tells the rest of the game about it.
	# Keep-awake is still worth setting; it doesn't care about orientation.
	if not is_web:
